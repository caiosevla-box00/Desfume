...
const INSIGHTS = [
  ...
  { tag:'Finanças', color:'tag-gold', title:'O cigarro é um imposto que você se cobra', body:'Cada maço comprado é uma escolha de gastar dinheiro com algo que encurta sua vida. Cada cigarro recusado é dinheiro no seu bolso.' },
  { tag:'Coração', color:'tag-ember', title:'Seu coração já está agradecendo', body:'Em 24 horas sem fumar, o risco de infarto começa a cair. Em 1 ano, é metade do de um fumante ativo. Seu coração está se recuperando agora.' },
  ...
  { tag:'Tempo', color:'tag-sky', title:'Você está recuperando tempo de vida', body:'Cada cigarro consome em média 5 minutos de vida. Você está recuperando esse tempo agora, segundo a segundo. Esse tempo é seu.' },
    ...
    <div className="page-padded">
      ...
      <div className="stat-grid" style={{marginBottom:'1rem'}}>
        <div className="stat-box">
          <div className="stat-val" style={{color:stats.todayCount>stats.cpd?'#DC2626':stats.todayCount>0?'#E05C2A':'#1A6B42'}}>{stats.todayCount}</div>
        ...
        <div className="stat-box">
          <div className="stat-val" style={{color:'#C47E00',fontSize:'1.1rem'}}>{brl(stats.moneySaved)}</div>
      ...
      {stats.reductionPct > 0 && (
        <div className="card card-mint" style={{marginBottom:'1rem'}}>
          ...
          <p style={{fontSize:'0.75rem',color:'#4A8C6F',marginTop:6}}>
            Fumou {num(stats.actualSmoked)} de {num(stats.expectedTotal)} esperados · {num(stats.cigsNotSmoked)} cigarros a menos
          </p>
        </div>
      )}

      {/* Extra smoked warning */}
      {stats.extraSmoked > 0 && (
        <div className="card card-red" style={{marginBottom:'1rem'}}>
          <p style={{fontSize:'0.8125rem',fontWeight:700,color:'#DC2626',marginBottom:4}}>⚠️ Você fumou além do seu padrão</p>
          <p style={{fontSize:'0.8125rem',color:'#6B8A74',lineHeight:1.5}}>
            {num(stats.extraSmoked)} cigarros a mais que o normal · gasto extra de {brl(stats.extraSpent)}
          </p>
        </div>
      )}
      ...
      <div className="card card-red" style={{marginBottom:'1rem'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
          <div style={{background:'white',borderRadius:8,padding:'10px',textAlign:'center'}}>
            <div style={{fontSize:'1.25rem',fontWeight:800,color:'#DC2626',lineHeight:1}}>{brl(stats.yearlyCost)}</div>
            <div style={{fontSize:'0.625rem',color:'#6B8A74',marginTop:3,fontWeight:600}}>em dinheiro</div>
          </div>
          <div style={{background:'white',borderRadius:8,padding:'10px',textAlign:'center'}}>
            <div style={{fontSize:'1.25rem',fontWeight:800,color:'#DC2626',lineHeight:1}}>{Math.round(stats.cpd*5*365/60/24)} dias</div>
      ...
      {/* Time recovered */}
      {stats.minutesRecovered > 0 && (
        <div className="card card-mint" style={{marginBottom:'1rem'}}>
          <p style={{fontSize:'0.75rem',fontWeight:700,color:'#0A3D2B',marginBottom:6}}>⏱️ Tempo de vida recuperado</p>
          <p style={{fontSize:'1.5rem',fontWeight:800,color:'#1A6B42',lineHeight:1}}>{stats.hoursRecovered}h</p>
          <p style={{fontSize:'0.75rem',color:'#4A8C6F',marginTop:3}}>{num(stats.minutesRecovered)} minutos · {num(stats.cigsNotSmoked)} cigarros não fumados</p>
        </div>
      )}
